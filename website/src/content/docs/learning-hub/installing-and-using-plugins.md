...
The `/plugins` command (v1.0.76+) opens an interactive panel where you can enable or disable individual components loaded from plugins — including plugins themselves, custom instructions, agents, LSP servers, and hooks — without uninstalling them:

```
/plugins
```

This is useful when you want to temporarily turn off a hook that is slowing down a session, or disable a set of instructions that conflict with a specific task, and then re-enable them afterward. Changes take effect immediately for the current session.
