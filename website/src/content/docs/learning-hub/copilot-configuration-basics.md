...
Models in the picker are grouped into **Recent**, **Recommended**, **New**, and other sections, making it easier to find the right model without scrolling through a long list. Use **Shift+Tab** to switch between grouping views.
...
**`worktreeBaseRef` setting**: Controls whether `/worktree`, `/worktree new`, and `--worktree` create the new branch starting from `HEAD` (the current state of your working tree) or from the remote default branch. All three default to `HEAD`. Set this in your user settings if you always want new worktrees to branch from the remote default instead:

```json
{
  "worktreeBaseRef": "remoteDefault"
}
```

| Value | Behavior |
|-------|----------|
| `"head"` (default) | New worktrees branch from the current HEAD |
| `"remoteDefault"` | New worktrees branch from the remote default branch (e.g., `main`) |
