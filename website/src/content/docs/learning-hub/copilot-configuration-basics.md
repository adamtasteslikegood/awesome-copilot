...
The `/worktree` command (v1.0.61+) creates a new git worktree and switches into it, **leaving your uncommitted changes behind** in the original worktree. Use it when you want to start a parallel branch from a clean state:
...
If you want to **carry your uncommitted changes into the new worktree**, use the `/move` command instead (v1.0.71+):

```
/move my-feature-branch
```

`/move` transfers your staged and unstaged changes to the new worktree so you can continue working on them in the new context. Use `/worktree` to start fresh and `/move` when you have in-progress work you want to bring along.
...
> **Plan mode workspace protection** (v1.0.71+): In plan mode, the agent can no longer edit files or run mutating shell commands — it is hard-blocked from using any built-in tool call that would modify your workspace. This makes plan mode a safe way to explore what changes would be made before authorizing them. Note that MCP and external tools are still allowed in plan mode; only Copilot's own workspace-mutating built-ins are blocked.
