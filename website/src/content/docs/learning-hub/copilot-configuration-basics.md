...
**Plan mode model** (v1.0.74+): Use `/model plan` (or `/model --plan`) to set a dedicated model for plan mode sessions, separate from your regular session model. This lets you use a lightweight model for planning and a more capable one for execution:

```
/model plan               # open the model picker scoped to plan mode
/model --plan gemini      # set Gemini as your plan-mode model
/model --plan off         # clear the plan-mode override
```

The plan-mode model automatically reverts to your regular session model when you exit plan mode.
...
*(v1.0.79+)* You can **combine `--plan` with `--mode autopilot`** to plan first and then implement automatically without waiting for approval between the two phases:

```bash
copilot --plan --mode autopilot "Refactor the authentication module"
```

This starts in plan mode, produces a plan, and then automatically switches to autopilot to execute it — ideal for well-scoped tasks where you trust the agent to proceed without mid-task intervention.
