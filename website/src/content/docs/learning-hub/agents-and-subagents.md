...
> **Note (v1.0.71+)**: The default maximum sub-agent nesting depth was lowered from 6 to 4 to reduce runaway recursive delegation. Usage-based billing users can raise `subagents.maxDepth` up to 128 in `/settings` if their workflows require deeper chains.
