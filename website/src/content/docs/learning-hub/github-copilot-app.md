...
*(v1.0.79+)* Use the `/app` command from within a Copilot CLI session to open that session directly in the GitHub Copilot desktop app (requires app 1.1.3 or later):

```
/app
```

This is useful when you start work in the CLI and want to continue in the visual Copilot app interface — for example, to view the canvas, approve changes visually, or manage the session from the My Work view. The app opens to the current session rather than landing on the Home screen.
